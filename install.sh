...
# 1Password 데이터 접근 테스트 (biometric 프롬프트 발생 가능)
echo "1Password 접근 테스트 중..."
if ! op read "op://Personal/Github-H8njo/username" &>/dev/null; then
  echo ""
  echo "1Password에서 인증이 필요합니다. (Face ID / Touch ID / 비밀번호)"
  echo ""
  # 다시 시도 - biometric 프롬프트 발생
  if ! op read "op://Personal/Github-H8njo/username" &>/dev/null; then
